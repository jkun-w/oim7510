...
@app.cell
def _():
    charge_1 = [16.75, 22.25, 25.00, 100.0, 200.0, 300.0]
    return (charge_1,)


@app.cell
def _(charge_1, cost_):
    for x in charge_1:
        tax_1 = x * .0625
        cost_1 = x + tax_1
        print(f'Pre-tax: ${x:.2f}, total: ${cost_:.2f}')
    return
...
@app.cell
def _(charges):
    # sum up all the carges that are under $25
    totalx = 0
    for chargex in charges:
        if chargex < 25:
            totalx += chargex
    print(f'Total of all charges under $25: {totalx}')
    return
...
app._unparsable_cell(
    r"""
    The first one will decide when score meets both tests at once
    """,
    name="_"
)
